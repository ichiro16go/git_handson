...
---
###　add,commit,pushの違いって、なーに？？
#### 1. git add
add は、ファイルをステージングエリアに追加する コマンドです。

ステージングエリア とは、コミットの準備をする場所です。編集したファイルをそのままではコミットできないので、まず「このファイルを次のコミットに含めますよ」と Git に教える作業が必要です。それが git add です。
#### 2. git commit
commit は、ステージングエリアに追加した変更をリポジトリに保存する コマンドです。

commit すると、その時点のファイルの状態が「スナップショット」として保存されます。これにより、後からそのバージョンに戻ったり、変更の履歴を確認したりすることができます。
#### 3. git push
push は、ローカルリポジトリのコミットをリモートリポジトリに送る コマンドです。

リモートリポジトリ（例：GitHubやBitbucketなど）は、他の人とコードを共有したり、別の場所からアクセスできるようにするためのものです。
push することで、ローカル（自分のPC）で保存された変更がリモートリポジトリに反映され、他の人もその変更を確認できるようになります。

![pushの説明](./push_pull.png)
![add,commitの説明](./add_commit.png)
---
